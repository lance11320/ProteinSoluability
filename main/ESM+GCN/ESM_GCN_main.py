...
# path
Dataset_Path = './Data/'
Model_Path = './ESMOnlyModel/'
Result_Path = './ESMOnlyResult/'
...
def load_features(sequence_name, sequence, mean, std, blosum):
    ...
    feature_matrix = np.load(Dataset_Path + 'node_feature_ESM/' + sequence_name + '.npy')
...
class GCN(nn.Module):
    ...
    def forward(self, x):  			# x.shape = (seq_len, GCN_FEATURE_DIM); adj.shape = (seq_len, seq_len)
        x = self.gc1(x)  				# x.shape = (seq_len, GCN_HIDDEN_DIM)
...
class Model(nn.Module):
    ...
    def forward(self, x):  											# x.shape = (seq_len, FEATURE_DIM); adj.shape = (seq_len, seq_len)
        x = x.float()
        x = self.gcn(x)  												# x.shape = (seq_len, GAT_OUTPUT_DIM)
